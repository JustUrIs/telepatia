...
var utf8Decoder, utf8Encoder, HEX, BufferShim;
...
function isTransaction(v) {
  return !!v && typeof v === "object" && typeof v.date === "string" && /^\d{4}-\d{2}-\d{2}$/.test(v.date) && typeof v.description === "string" && Number.isFinite(v.amount) && typeof v.currency === "string" && v.currency.length === 3;
}
function isCheckResult(v) {
  return !!v && typeof v === "object" && typeof v.id === "string" && typeof v.ok === "boolean" && Array.isArray(v.evidence);
}
function isVerdict(v) {
  return !!v && typeof v === "object" && ["pass", "fail", "review"].includes(v.verdict) && Array.isArray(v.checks) && v.checks.every(isCheckResult) && Array.isArray(v.ungrounded) && (v.matched === null || isTransaction(v.matched));
}
...
function inflateRawSync(data) {
  ...
  for (; ; ) {
    ...
    const tipo = cabecera >>> 1 & 3;
    if (tipo !== 0) {
      throw new Error(
        "este shim solo descomprime bloques stored: el stream trae deflate real (BTYPE=" + tipo + "). Armalo con el receptor de Node."
      );
    }
    const largo = entrada[cursor + 1] | entrada[cursor + 2] << 8;
    const complemento = entrada[cursor + 3] | entrada[cursor + 4] << 8;
    if ((largo ^ 65535) !== complemento) throw new Error("LEN y NLEN no se corresponden");
    cursor += 5;
...
var FrameDecoder = class {
  ...
  push(bytes) {
    ...
    if (frame.unsupportedVersion !== void 0) {
    ...
    switch (frame.kind) {
      case KIND.MANIFEST:
        return this.#pushManifest(frame);
      case KIND.DATA:
        return this.#pushData(frame);
      case KIND.PARITY:
        return this.#pushParity(frame);
      default:
        this.#stats.foreign++;
        return false;
    }
  ...
  #pushManifest(frame) {
    ...
    try {
      ...
      if (typeof manifest.sha256 !== "string" || !/^[0-9a-f]{64}$/.test(manifest.sha256)) {
        throw new TypeError("manifest.sha256 inv\xE1lido");
      }
    ...
    for (const [index, payload] of [...this.#chunks]) {
      const ok = index < manifest.total && payload.length === this.#expectedPayloadLen(KIND.DATA, index);
  ...
  #recover() {
    ...
    while (cambio) {
      ...
      for (const [w, paridad] of this.#parity) {
        ...
        const recuperado = BufferShim.from(paridad);
        for (let i = inicio; i < fin; i++) {
          const chunk = this.#chunks.get(i);
          if (chunk === void 0) continue;
          for (let j = 0; j < chunk.length; j++) recuperado[j] ^= chunk[j];
        }
        const largoReal = faltante === total - 1 ? bodyLength - faltante * chunkSize : chunkSize;
  ...
  assemble() {
    if (this.#manifest === null) {
      throw new Error("todav\xEDa no lleg\xF3 el manifest del documento");
    }
    const manifest = this.#manifest;
    if (!this.complete) {
    ...
    const body = BufferShim.concat(partes);
    ...
    if (manifest.compression === COMPRESSION.DEFLATE_RAW) {
      try {
        ...
        throw failure(FAILURE_CODES.digestMismatch, "el body no descomprime: lleg\xF3 corrupto");
      }
    } else {
      document = body;
    }
    ...
    const sha2562 = createHash("sha256").update(document).digest("hex");
    if (sha2562 !== manifest.sha256) {
...
var jsQR = import_jsqr.default.default ?? import_jsqr.default;
function scanRgba(data, width, height) {
  if (!ArrayBuffer.isView(data)) {
    throw new TypeError("rgba debe ser Uint8ClampedArray o Uint8Array");
  }
  if (!Number.isInteger(width) || !Number.isInteger(height) || width < 1 || height < 1) {
    throw new RangeError(`dimensiones inv\xE1lidas: ${width}x${height}`);
  }
  if (data.length !== width * height * 4) {
    throw new RangeError(
      `dimensiones inconsistentes: ${width}x${height} pide ${width * height * 4} bytes y el buffer trae ${data.length}`
    );
  }
...
var ScanLoop = class {
  ...
  constructor(provider, decoder) {
    if (typeof provider !== "function") {
      throw new TypeError("provider debe ser una funci\xF3n que devuelva {rgba, width, height} o null");
    }
  ...
  run(maxTicks) {
    if (!Number.isInteger(maxTicks) || maxTicks < 1) {
      throw new RangeError(`maxTicks inv\xE1lido: ${maxTicks}`);
    }
    this.#stopped = false;
...
var ETIQUETAS_CHECK = {
  ...
  tax_amount_matches_rate: "El impuesto corresponde a la al\xEDcuota",
  tax_rate_plausible: "La al\xEDcuota es plausible",
  currency_consistent: "La moneda es consistente",
  dates_ordered: "Emisi\xF3n no posterior al vencimiento",
  issue_date_not_future: "La emisi\xF3n no est\xE1 en el futuro",
  invoice_not_duplicate: "La factura no est\xE1 duplicada"
};
var ETIQUETAS_CAMPO = {
  invoiceNumber: "N\xFAmero de factura",
...
var ESTADOS = {
  ...
  review: {
    title: "Requiere revisi\xF3n",
    tone: "warn",
    summary: "Nada la contradice, pero falta respaldo: revisi\xF3n humana antes de aprobar."
  }
};
var FALLOS = {
  [FAILURE_CODES.digestMismatch]: {
    title: "El documento lleg\xF3 corrupto",
    detail: "El SHA-256 de lo reconstruido no coincide con el que declara el emisor. Volv\xE9 a escanear: no se descarta nada, se rehace la captura."
  },
  [FAILURE_CODES.ungroundedFields]: {
    title: "Hay campos sin respaldo en el documento",
    detail: "El modelo extrajo valores que ning\xFAn bloque del documento sostiene. Esos campos van marcados y el dictamen queda en revisi\xF3n."
  },
  [FAILURE_CODES.unsupportedVersion]: {
    title: "El emisor habla una versi\xF3n m\xE1s nueva",
    detail: "El stream usa una versi\xF3n del protocolo AGP1 que este receptor no entiende. Actualiz\xE1 el receptor o baj\xE1 la versi\xF3n del emisor."
  },
  [FAILURE_CODES.emptyDocument]: {
    title: "El documento est\xE1 vac\xEDo",
    detail: "No hay bytes que auditar. Revis\xE1 el archivo del lado del emisor."
  },
  [FAILURE_CODES.backendUnavailable]: {
    title: "El motor de inferencia no est\xE1 disponible",
    detail: "No se pudo cargar el modelo local. El transporte funcion\xF3: el documento est\xE1 reconstruido y se puede auditar despu\xE9s."
  },
  [FAILURE_CODES.malformedExtraction]: {
    title: "La extracci\xF3n no respet\xF3 el esquema",
    detail: "El modelo devolvi\xF3 algo que no encaja en el esquema de factura. El dictamen no se emite con una extracci\xF3n malformada."
  },
  [FAILURE_CODES.malformedRow]: {
    title: "Hay filas del extracto que no se pudieron leer",
    detail: "Alguna fila del CSV no normaliza. No se descarta en silencio: queda listada con su motivo."
  },
  [FAILURE_CODES.cameraUnavailable]: {
    title: "No hay c\xE1mara disponible",
    detail: "El permiso fue denegado o no hay ning\xFAn dispositivo de video. Revis\xE1 los permisos del sitio y que ninguna otra app tenga la c\xE1mara tomada."
  }
};
var porcentaje = (v) => `${Math.round((Number(v) || 0) * 100)}%`;
function renderVerdict(verdict) {
  if (!isVerdict(verdict)) {
    throw new TypeError("renderVerdict espera un Verdict v\xE1lido del contrato");
  }
  ...
  return {
    ...
    matchLabel: verdict.matched ? `${verdict.matched.date} \xB7 ${verdict.matched.description} \xB7 ${verdict.matched.amount}` : "Sin movimiento asociado en el extracto"
  };
}
function renderFailure(failure2) {
  const stage = typeof failure2?.stage === "string" ? failure2.stage : "desconocida";
  const code = typeof failure2?.code === "string" ? failure2.code : "";
  const message = typeof failure2?.message === "string" ? failure2.message : "";
  const conocido = Object.hasOwn(FALLOS, code);
  const base = conocido ? FALLOS[code] : {
    title: "Fallo no clasificado",
    detail: `El pipeline se cort\xF3 en la etapa "${stage}" con un c\xF3digo que este receptor no conoce. Se muestra el mensaje original abajo.`
  };
  return {
    stage,
    code,
...
function mount(doc = globalThis.document) {
  ...
  function pintarFallo(failure2) {
    const vista = renderFailure(failure2);
    panel.hidden = false;
  ...
  function pintarDictamen(informe) {
    ...
    for (const fila of vista.rows) {
      ...
      evidencia.textContent = fila.evidence.map((ev) => `${ev.value} (${ev.confidenceLabel})`).join(" \xB7 ");
...
\u2026 recortado, son ${vista.lines} l\xEDneas en total` : vista.text;
  ...
  function completar() {
    ...
    try {
      ...
      setEstado("el documento lleg\xF3 corrupto", "error");
  ...
  function bucle() {
    ...
    if (decoder.unsupportedVersion !== null) {
      corriendo = false;
      setEstado("versi\xF3n de protocolo no soportada", "error");
      pintarFallo({
        stage: "scan",
